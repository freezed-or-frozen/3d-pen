...
class Passerelle(WebSocket):

    def handleMessage(self):
        # echo message back to client
        #self.sendMessage(self.data)
        print(" => Réception et envoi de :", self.data)
        self.sc.send(self.data.encode('utf-8'))

    def handleConnected(self):
        print(" => Connexion du smartphone :", self.address)
        # Connexion au serveur Processing
        print(" => Connexion au serveur processing sur le port 1665")
        self.sc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sc.connect(("127.0.0.1", 1665))
        message = "HELLO WORLD !!!"
        self.sc.send(message.encode('utf-8'))
        

    def handleClose(self):
        print(" => Déconnexion du smartphone :", self.address)

# Initialisation du serveur websocket pour le smartphone
print(" => Initialisation du serveur pour le smartphone")
server = SimpleWebSocketServer('', 1664, Passerelle)
print(" => Serveur pour le smartphone en écoute")
server.serveforever()
